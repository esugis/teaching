...
Loading spreadsheets can be done by using basic commands:
`read.csv()`, `read.csv2()` and `and read.table()`
...
There might be another separator symbol used in your file instead of comma, for exapmle a semicolon.
To load the files where fields are separated by a semicolon, you can use `read.csv2()` function.
...
Most of the time you don’t want to include row names in the CSV.
To omit rownames add row.names = FALSE.
...
```{r, echo = TRUE, eval = FALSE}
# Save in .csv format
write.table(df, file = "results/mydata.csv", sep = ",", row.names = FALSE, col.names = TRUE, quote = FALSE, na = "")
```

```{r, echo = TRUE, eval = FALSE}
# Save in .txt (tab delimited) format
write.table(df, file = "results/mydata.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE, na = "")
```
