...
#download.file("https://s3-eu-west-1.amazonaws.com/pfigshare-u-files/5515439/data_preprocessed.txt", "data/data_preprocessed.txt")
```
...
```{r}
data <- read.table(file = "data/data_preprocessed.txt", sep = "\t", header = TRUE)
```
...
# Cluster your data
In biology everything is connected, and there is a very little space for coincidences. Similar things act in a similar way. As we study gene expression, we might be potentially interested in capturing genes that behave in similar fashion. In order to find potentially interesting groups of genes, we would perform a cluster analysis. 
...
In order to work with `kmeans` clustering, one has to choose an appropriate number of clusters, which he/she anticipates to find in the data. To estimate the number of clusters, Elbow method/plot is used (check https://en.wikipedia.org/wiki/Determining_the_number_of_clusters_in_a_data_set). 
